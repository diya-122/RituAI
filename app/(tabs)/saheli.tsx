...
export default function Saheli() {
  ...
  const send = async (content: string) => {
    ...
    try {
      const currentHistory = useStore.getState().chat;
      const response = await api.chat(currentHistory.map(m => ({ role: m.role, content: m.content })));
      
      let replyText = "I'm sorry, I'm having trouble connecting to my thoughts right now.";
      if (response && response.status === 'success' && response.reply) {
        replyText = response.reply;
      } else if (response && response.message) {
        replyText = `Error: ${response.message}`;
      }
      ...
      addChatMessage({
        id: `s-${Date.now()}`,
        role: 'saheli',
        content: "I'm offline right now, but please take a deep breath and take care of yourself.",
        timestamp: new Date().toISOString(),
      });
    } finally {
      setIsTyping(false);
