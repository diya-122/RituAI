// Thin API client — calls FastAPI when reachable, otherwise returns mocks.
// Set EXPO_PUBLIC_API_BASE in app.json extra or env. Safe default so the app never crashes.
...
async function safeFetch<T>(path: string, init?: RequestInit, fallback?: T): Promise<T | null> {
  try {
    ...
    // Silent fallback — app never blocks on network
